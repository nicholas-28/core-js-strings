...
function startsWith(str, substr) {
  const isStarts = str.startsWith(substr);
  return isStarts;
}
...
function endsWith(str, substr) {
  const isEnds = str.endsWith(substr);
  return isEnds;
}
...
function orderAlphabetically(str) {
  const alphaOrder = str.split('').sort().join('');
  return alphaOrder;
}
...
function containsSubstring(str, substring) {
  const isIncludes = str.includes(substring);
  return isIncludes;
}
